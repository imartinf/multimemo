...
from src.tools.video_processing import (
    sample_center_segment,
    sample_frame_indices,
    sample_salient_segment,
    collect_all_possible_segments,
)
...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def _get_item(self, idx):
        ...
        """
        # Is eval is true, search for all segments with the same filename and return them as a list
        if self.eval:
            self.eval = False
            video_col = "video" if "video" in self.data.columns else "filename"
            # Get filename without extension
            filename = os.path.splitext(os.path.basename(self.videos[idx]))[0]
            # Get all rows with the same filename
            rows = self.data[self.data[video_col].str.contains(filename)]
            # Call _get_item for each row and append the result to a list
            video = []
            for _, row in rows.iterrows():
                video.append(self._get_item(row.name))
            if len(video) == 1:
                video = video[0]
            self.eval = True
            return video
        ...
        if self.frame_sample_strategy == "all-segments":
            if not ("start_idx" in self.data.columns and "end_idx" in self.data.columns):
                indices = collect_all_possible_segments(self.clip_len, container.streams.video[0].frames, 15)
                indices = [list(range(start, end)) for start, end in indices]
            else:
            ...
            indices = sample_salient_segment(
                self.clip_len, container.streams.video[0].frames, self.saliency_scores[self.videos[idx]]
            )
        else:
            raise ValueError(f"Invalid frame sample strategy: {self.frame_sample_strategy}")

        video = (
            [self._read_video_pyav(container, seg_indices) for seg_indices in indices]
            if isinstance(indices[0], list)
            else self._read_video_pyav(container, indices)
        )
        ...
        for i, segment in enumerate(video):
            if len(segment) < self.clip_len:
                ...
                logging.warning(f"Video shape: {segment.shape}")
                logging.warning(f"Indices: {indices[i]}")
                logging.warning(f"Container frames: {container.streams.video[0].frames}")
                segment = np.concatenate(
                    [np.repeat(segment[-1:], self.clip_len - len(segment), axis=0), segment], axis=0
                )
                video[i] = segment
            assert (
                len(video[i]) == self.clip_len
            ), f"Video {self.videos[idx]} has {len(video[i])} frames instead of {self.clip_len}."

        return video if not self.return_video_id else (video, self.videos[idx])
        ...
        if not self.eval:
            ...
            for i, segment in enumerate(video):
                if isinstance(segment, tuple):
                    segment = segment[0]
                inputs = self.processor(list(segment), return_tensors="pt")
                inputs = {k: val.squeeze() for k, val in inputs.items()}
                # Get label
                label = torch.tensor(self.labels[idx]).float()
                video[i] = (inputs, label)
            video_id = os.path.splitext(os.path.basename(self.videos[idx]))[0]
            return video if not self.return_video_id else (video, video_id)
